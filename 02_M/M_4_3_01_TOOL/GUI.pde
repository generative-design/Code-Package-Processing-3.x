...
void setupGUI(){
  ...
  sliders[si++] = controlP5.addSlider("layerCount",1,9,left,top+posY,len,15);
  posY += 30;
  ...
  sliders[si++] = controlP5.addSlider("gridStepX",1,100,left,top+posY,len,15);
  sliders[si++] = controlP5.addSlider("gridStepY",1,100,left,top+posY+20,len,15);
  posY += 50;

  toggles[ti] = controlP5.addToggle("attractorSmooth",attractorSmooth,left+0,top+posY,15,15);
  toggles[ti++].setLabel("Attractor Function Normal");
  toggles[ti] = controlP5.addToggle("attractorTwirl",attractorTwirl,left+0,top+posY+20,15,15);
  toggles[ti++].setLabel("Attractor Function Twirl"); 
  posY += 40;
  ...
  infoBang = controlP5.addBang("info", width-160, 12,0,15);
  infoBang.setLabel("Currently affected layer: 1"); 
  posY += 10;

  infoBang.setColorCaptionLabel(color(50));
  infoBang.getCaptionLabel().getStyle().padding(4,3,3,3);
  infoBang.getCaptionLabel().getStyle().marginTop = -20;
  infoBang.getCaptionLabel().getStyle().marginLeft = 3;
  infoBang.getCaptionLabel().getStyle().marginRight = 5;
  infoBang.getCaptionLabel().setColorBackground(0x99ffffff);
...
void controlEvent(ControlEvent theControlEvent) {
  ...
  if(theControlEvent.isController()) {
    if(theControlEvent.getController().getName().equals("layerCount")) {
      actLayer = min(actLayer, layerCount-1);
      infoBang.setLabel("Currently affected layer: " + (actLayer+1)); 
    }
...
void attractorSmooth() {
  if (!guiEvent) {
    guiEvent = true;
    attractorSmooth = !attractorSmooth;
    attractorTwirl = !attractorSmooth;
    Toggle t = (Toggle) controlP5.getController("attractorTwirl");
    t.setState(attractorTwirl);
  }
}
...
void attractorTwirl() {
  if (!guiEvent) {
    guiEvent = true;
    attractorTwirl = !attractorTwirl;
    attractorSmooth = !attractorTwirl;
    Toggle t = (Toggle) controlP5.getController("attractorSmooth");
    t.setState(attractorSmooth);
  }
}
